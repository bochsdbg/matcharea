...
bool CStrokePattern::isEqual(CAbstractPattern *other) {
    ...
    if (pattern) {
        if (strekojKvanto() != pattern->strekojKvanto()) return false;
        for (int i = 0; i < strekojKvanto(); ++i) {
            if (strekoj.at(i) != pattern->streko(i)) return false;
        }
    }
    return true;
}
...
void CStrokePattern::drawTo(QPainter *painter, const QRect &drawRect, const QColor &color) {
    ...
    for (int i = 0; i < strekoj.count(); ++i) {
        int m = strekoj[i];
        int largho = bildRekt.width() / m;
        int blokLargho = largho;
        int blokShovo = 0;
        if (strekoj[i] > 1) {
            blokLargho = qRound(largho * 0.9);
            blokShovo = largho + (largho - blokLargho) / (m - 1);
        }
        for (int j = 0; j < strekoj[i]; ++j) {
            painter->drawRect(QRect(bildRekt.left() + j * blokShovo, bildRekt.top() + i * blokAlto + altShovo,
                                    blokLargho, strekAlto));
        }
        // egaligi dekstran randon
        painter->drawRect(
            QRect(bildRekt.right() - blokLargho + 1, bildRekt.top() + i * blokAlto + altShovo, blokLargho, strekAlto));
    }
}
